        ...
        for configuration in map_configurations:
            for moment_in_time in moments_in_time:
                for filter in filters:
                    log.debug("%s %s %s %s" % (configuration.country, configuration.organization_type.name,
                                               filter, moment_in_time))

                    map_only_url = config.PROJECT_WEBSITE + "/map_only/%s/%s/%s/%s/" % (
                        configuration.country, configuration.organization_type.name, moment_in_time, filter)
...
def screenshot_firefox(url, filename):
    command = [firefox, '-headless', '-screenshot', filename, url, '--window-size=1920,1080', '-P', 'screenshots',
               '-no-remote']
