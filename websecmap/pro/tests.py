...
def test_explanations(db):
    ...
    scan2, created = EndpointGenericScan.objects.all().get_or_create(
        endpoint=endpoint, type="plain_https", rating="F", is_the_latest_scan=True, rating_determined_on=timezone.now()
    )
    ...
    assert (
        explained["success"] is False
        and explained["message"] == "This account does not have enough credits to "
