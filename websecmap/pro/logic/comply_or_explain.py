...
def get_scan(account: Account, scan_id: int, scan_type: str) -> Union[EndpointGenericScan, UrlGenericScan, None]:
    ...
    if scan_type in ENDPOINT_SCAN_TYPES:
        return (
            ...
            .filter(pk=scan_id, type=scan_type, endpoint__url__urllist__account=account)
            .first()
        )

    if scan_type in URL_SCAN_TYPES:
        return UrlGenericScan.objects.all().filter(pk=scan_id, type=scan_type, url__urllist__account=account).first()
...
def get_scan_data(account: Account, scan_id: int, scan_type: str):
    ...
    impact = (
        "high"
        if severity.get("high", 0)
        else "medium"
        if severity.get("medium", 0)
        else "low"
        if severity.get("low", 0)
        else "ok"
    )
