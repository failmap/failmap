...
class Account(models.Model):
    ...
    name = models.CharField(max_length=120, blank=True, null=True, help_text="Understandable name of the organization.")

    enable_logins = models.BooleanField(
        blank=True, null=True, default=False, help_text="Inactive accounts cannot be logged-in to."
    )
...
class CreditMutation(models.Model):
    ...
    credits_received = models.PositiveIntegerField(
        blank=True, null=True, default=0, help_text="A positive number of amount of credits received."
    )

    credits_spent = models.PositiveIntegerField(
        blank=True, null=True, default=0, help_text="A positive number of the amount of credits spent."
    )
...
class UrlList(models.Model):
    ...
    send_notification_on_new_issue_with_medium_risk = models.BooleanField(default=False)

    send_notification_on_new_issue_with_low_risk = models.BooleanField(default=False)
    ...
    send_report_format = models.CharField(
        max_length=40, choices=[("html_inline", "Inline HTML")], default="html_inline"
    )
...
class FailmapOrganizationDataFeed(models.Model):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, help_text="Organization where urls come from."
    )
...
class RescanRequest(models.Model):
    ...
    cost = models.PositiveIntegerField(
        blank=True, null=True, default=0, help_text="A positive number of the amount of credits spent for this re-scan."
    )
    ...
    scan_id = models.IntegerField(default=0, help_text="Makes it easy to find if a rescan is already requested.")

    endpoint_scan = models.ForeignKey(EndpointGenericScan, on_delete=models.CASCADE, null=True)

    url_scan = models.ForeignKey(UrlGenericScan, on_delete=models.CASCADE, null=True)
