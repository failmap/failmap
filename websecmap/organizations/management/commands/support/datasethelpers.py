...
def sqlite_has_correct_referential_integrity():
    ...
    log.info("Checking for foreign key issues, and generating possible SQL to remediate issues.")
    ...
    if rows:
        log.error(
            "Cannot create export. There are incomplete foreign keys. "
    ...
    if rows:
        log.error("Here are some extremely crude SQL statements that might help fix the problem.")
    for row in rows:
...
def check_referential_integrity():
    ...
    if "sqlite" in settings.DATABASES["default"]["ENGINE"]:
        ...
        log.error(
            "This export might have incorrect integrity: no foreign key check for "
