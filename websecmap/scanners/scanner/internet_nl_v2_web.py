...
def compose_planned_scan_task(**kwargs):
    urls = plannedscan.pickup(activity="scan", scanner="internet_nl_web", amount=kwargs.get("amount", 500))
...
def plan_scan(
    ...
    plannedscan.request(activity="scan", scanner="internet_nl_web", urls=urls)
...
def compose_scan_task(urls):
    ...
    return group([initialize_scan.si("web", urls) | plannedscan.finish_multiple.si("scan", "internet_nl_web", urls)])
