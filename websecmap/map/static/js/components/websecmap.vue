    ...
    <div :class="loading ? 'websecmap map_loading' : 'websecmap'">
        ...
        <l-map style="height: 100vh; width: 100%;" ref="lmap"
            ...
            <l-control position="topright" v-if="issues.length > 1" class="hide_on_small_screens">
                <div style="max-width: 300px; overflow:hidden;" class="info table-light">
                    <button class="btn btn-secondary btn-small" style='width: 100%' type="button" data-toggle="collapse"
                            data-target="#collapseFilters" aria-expanded="false" aria-controls="collapseExample">
                        <template v-if="['clear_filter_option', ''].includes(displayed_issue)">{{
                                $t("map.filter.title")
                            }}
                        </template>
                        <template v-if="displayed_issue !== ''">
                            <span v-if='loading'><div class="loader"
                                                      style="width: 24px; height: 24px; float: left;"></div></span>
                            {{ $t(displayed_issue) }}
                        </template>
                    </button>

                    <div class="collapse" id="collapseFilters" style="margin-top:10px;">
                        <input type='radio' v-model="displayed_issue" name="displayed_issue" value=""
                        ...
                        <div v-for="issue in issues" style="white-space: nowrap;">
                            <input type='radio' v-model="displayed_issue" name="displayed_issue" :value="issue.name"
                                   :id="issue.name">
                        ...
                        <div v-if="domainlist_urls.length > 0">
                            <table width='100%'>
                                ...
                                <tr v-for="url in domainlist_urls">
                                    <td><span :class="colorize(url.high, url.medium, url.low)+'_text'">{{
                                            url.high
                                        }}</span></td>
                                    <td><span :class="colorize(url.high, url.medium, url.low)+'_text'">{{
                                            url.medium
                                    ...
                                    <td nowrap><span :class="colorize(url.high, url.medium, url.low)+'_text'">{{
                                            url.url
                                        }}</span></td>
                                </tr>
                                </tbody>
                ...
                <div style="max-width: 300px; overflow:hidden;" class="info table-light">
                    ...
                    <template v-if="simplestats[this.state.country][this.state.layer] !== undefined">
                        ...
                        <template v-if="Object.keys(simplestats).length > 2">
                            <h4><img
                                :src="simplestats[this.state.country][this.state.layer].country_flag"/>{{ simplestats[this.state.country][this.state.layer].country_name }},
                                {{ $t($store.state.layer) }}</h4>
                        </template>
                        <template v-else>
                            <h4>{{ $t($store.state.layer) }}</h4>
                        </template>
                        {{ simplestats[this.state.country][this.state.layer].organizations }}
                        {{ $t('organizations') }}<br>
            ...
            <l-control position="topleft">
                <div>
                    <span @click='start_showing_map_health()'
                          style='margin-top: 34px; font-size: 1.2em; background-color: white; border: 2px solid rgba(0,0,0,0.35); border-radius: 4px; padding: 6px; height: 34px; position: absolute; width: 34px; text-align: center; line-height: 1.2;'>
                        <template v-if="map_health">
                            ...
                            <template
                                v-if="map_health['percentage_up_to_date'] > 0 && map_health['percentage_up_to_date'] < 55">🔴</template>
                            <template
                                v-if="map_health['percentage_up_to_date'] > 54 && map_health['percentage_up_to_date'] < 90">🟠</template>
                            <template
                                v-if="map_health['percentage_up_to_date'] > 89 && map_health['percentage_up_to_date'] < 100">🟢</template>
        ...
        <modal v-if="show_map_health" @close="stop_showing_map_health()">
            ...
            <div slot="body">
                <p>Map transparency gives insight into how up to date this map is and if there are pending scans
                    scheduled. Data is considered out of date after {{ map_health['outdate_period_in_hours'] }} hours.
                    Scans of the past 7 days are shown.</p>
                <div class="container">
                    <div class="row">
                        <div class="col-sm">
                            ...
                            <table style="width: 100%">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <template
                                            v-if="map_health['percentage_up_to_date'] > 0 && map_health['percentage_up_to_date'] < 55">
                                            🔴 Outdated
                                        </template>
                                        <template
                                            v-if="map_health['percentage_up_to_date'] > 54 && map_health['percentage_up_to_date'] < 90">
                                            🟠 Workable
                                        </template>
                                        <template
                                            v-if="map_health['percentage_up_to_date'] > 89 && map_health['percentage_up_to_date'] < 100">
                                            🟢 Usable
                                        </template>
                                        <template v-if="map_health['percentage_up_to_date'] === 100">🌈 Up to date
                                        </template>
                                    </td>
                                </tr>
                                <template v-if="map_health['percentage_up_to_date'] !== -1">
                                    <tr>
                                        <td>Up to date:</td>
                                        <td>{{ map_health['percentage_up_to_date'] }}%
                                            ({{ map_health['amount_up_to_date'] }})
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>Out of date:</td>
                                        <td>{{ map_health['percentage_out_of_date'] }}%
                                            ({{ map_health['amount_out_of_date'] }})
                                        </td>
                                    </tr>
                                </template>
                            </table>
                            ...
                            <table style="width: 100%">
                                <template v-for="scan in map_health['per_scan']">
                                    <tr>
                                        <td colspan="2"><small>{{ $t(scan['scan_type']) }}</small></td>
                                    </tr>
                                    <tr>
                                        <td><small>Up to date:</small></td>
                                        <td><small>{{ scan['percentage_up_to_date'] }}%
                                            ({{ scan['amount_up_to_date'] }})</small></td>
                                    </tr>
                                    <tr>
                                        <td><small>Out of date:</small></td>
                                        <td><small>{{ scan['percentage_out_of_date'] }}%
                                            ({{ scan['amount_out_of_date'] }})</small></td>
                                    </tr>
                                    <tr></tr>
                                </template>
                            </table>
                        ...
                        <div class="col-sm">
                            ...
                            <table width="100%;">
                                <template v-for="(scanner_value, scanner) in planned_scan_progress">
                                    <tr>
                                        <td colspan="2"><b>{{ $t(PSscanners[scanner]) }}</b></td>
                                    </tr>
                                    <template v-for="(metrics, scanner_task) in scanner_value">
                                        <tr>
                                            <td style="width: 30%;">
                                                <i>{{ PSactivity[scanner_task] }}</i>
                                            </td>
                                            <td style="width: 80%;">
                                                <div style="width: 100%; background-color: gray; height: 15px;">
                                                    <template v-for="(value, metric) in metrics">
                                                        <div v-if='metric === "3"'
                                                             :style="'width: ' + Math.floor((value / metrics['total']) * 100) + '%; background-color: darkgreen; color: white; float: left; text-align: center; vertical-align: middle; height: 15px; font-size: 0.7em;'">
                                                            {{ value }}
                                                        </div>
                                                        <div v-if='metric === "1"'
                                                             :style="'width: ' + Math.floor((value / metrics['total']) * 100) + '%; background-color: lightgreen; color: white; float: left; text-align: center; vertical-align: middle;  height: 15px;  font-size: 0.7em;'">
                                                            {{ value }}
                                                        </div>
                                                        <div v-if='metric === "2"'
                                                             :style="'width: ' + Math.floor((value / metrics['total']) * 100) + '%; background-color: orange; color: white; float: left; text-align: center; vertical-align: middle;  height: 15px; font-size: 0.7em;'">
                                                            {{ value }}
                                                        </div>
                                                    </template>
                                                </div>
                            ...
                            <table>
                                <tr>
                                    <td>
                                        <div style="height: 15px; width:15px; background-color: darkgreen;">&nbsp;</div>
                                    </td>
                                    <td>Scan finished</td>
                                </tr>
                                <tr>
                                    <td>
                                        <div style="height: 15px; width:15px; background-color: lightgreen;">&nbsp;
                                        </div>
                                    </td>
                                    <td>Planned</td>
                                </tr>
                                ...
                                <tr>
                                    <td>
                                        <div style="height: 15px; width:15px; background-color: gray;">&nbsp;</div>
                                    </td>
                                    <td>Other (errors, timeouts)</td>
                                </tr>
                            </table>
        ...
        <modal v-if="show_add_domains" @close="stop_adding_domains()">
            ...
            <div slot="body">
                ...
                <p><i>Note: the urls will be onboarded and scanned afterwards. It will be only be visible when endpoints
                    are found that we understand. The url will only be visible after a new report has been created with
                    aforementioned data. This can take a day.</i></p>
                <server-response :response="add_domains_server_response"></server-response>
                <h4>New domains</h4>
                <textarea style="width: 100%; height: 140px" v-model="new_domains"
                          placeholder="example.com, test.nl, Every domain on a new line, or separated with comma's."></textarea>
            </div>
            <div slot="footer">
    ...
    methods: {
        ...
        get_simplestats: function () {
            fetch(`/data/short_and_simple_stats/${this.state.week}/`).then(response => response.json()).then(data => {
                ...
                console.log('A simplestat loading error occurred: ' + fail)
            });
        },
        ...
        get_planned_scan_progress: function () {
            ...
            fetch(`/data/planned_scan_progress/`).then(response => response.json()).then(data => {
                ...
                console.log('A planned scan process loading error occurred: ' + fail)
            });
        },
        ...
        highlightFeature: function (e) {
            this.timer = setTimeout(() => {
                ...
                fetch(url).then(response => response.json()).then(data => {
                    ...
                    console.log('A domainlist loading error occurred: ' + fail);
                });
        ...
        style: function (feature) {
            return {
                weight: 1, opacity: 1, color: 'white', dashArray: '0', fillOpacity: 0.7,
